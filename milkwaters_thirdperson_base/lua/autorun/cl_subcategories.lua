if CLIENT then
	...
	hook.Add("PopulateWeapons", "MW_SubCategories", function(pnlContent, tree, anode)
		local cvar = 1
		...
		timer.Simple(0, function()
			...
			for k, weapon in pairs(Weapons) do
				...
				if not weapons.IsBasedOn(k, "milkwaters_3p_base") then continue end
				...
				node.DoPopulate = function(self)
					...
					for subcatName, subcatWeps in SortedPairs(catSubcats) do
						...
						for _, ent in ipairs(subcatWeps) do
							spawnmenu.CreateContentIcon("mw_weapon", self.PropPanel, {
								nicename = ent.PrintName or ent.ClassName,
								spawnname = ent.ClassName,
								material = ent.IconOverride or "entities/" .. ent.ClassName .. ".png",
								admin = ent.AdminOnly,
	...
	hook.Add("InitPostEntity", "MW_OverrideSpawnmenuReloadSWEP", function()
		hook.Add("PreRegisterSWEP", "spawnmenu_reload_swep", function(weapon, name)
			if not weapon.Spawnable or weapons.IsBasedOn(name, "milkwaters_3p_base") then return end
